...
int loader::error(int errorCode, int captionCode)
{
    ...
    pb::ShowMessageBox(1, errorCaption, errorText);
    return -1;
}
        ...
        if (soundGroupId > 0 && !pb::quickFlag)
        {
            ...
            if (value && *value == 202)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    {
                        filePath = filePath.substr(0, filePath.find_last_of('.')) + ".wav64";
                    }

                    sound_list[soundIndex].WavePtr = Sound::LoadWaveFile(filePath);
                    if (sound_list[soundIndex].WavePtr)
                    {
                        wav64_t *wav = sound_list[soundIndex].WavePtr;
                        float duration = wav->wave.len / wav->wave.frequency;
                    ...
                    //     wavHeader.overall_size = __builtin_bswap32(wavHeader.overall_size);
                    //     wavHeader.length_of_fmt = __builtin_bswap32(wavHeader.length_of_fmt);
                    //     wavHeader.format_type = __builtin_bswap16(wavHeader.format_type);
                    //     wavHeader.channels = __builtin_bswap16(wavHeader.channels);
                    //     wavHeader.sample_rate = __builtin_bswap32(wavHeader.sample_rate);
                    //     wavHeader.byterate = __builtin_bswap32(wavHeader.byterate);
                    //     wavHeader.block_align = __builtin_bswap16(wavHeader.block_align);
                    //     wavHeader.bits_per_sample = __builtin_bswap16(wavHeader.bits_per_sample);
                    //     wavHeader.data_size = __builtin_bswap32(wavHeader.data_size);

                    //     float sampleCount = wavHeader.data_size / (wavHeader.channels * (wavHeader.bits_per_sample /
                    //                                                                      8.0));
