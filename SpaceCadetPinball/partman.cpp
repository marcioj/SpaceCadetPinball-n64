...
DatFile *partman::load_records(LPCSTR lpFileName, bool fullTiltMode)
{
    ...
    header = LReadDatFileHeader(fileHandle);
    ...
    {
        auto entryCount = LRead8(fileHandle);
        ...
        for (auto entryIndex = 0; entryIndex < entryCount; ++entryIndex)
        {
            ...
            auto entryType = static_cast<FieldTypes>(LRead8(fileHandle));
            entryData->EntryType = entryType;

            int fixedSize = _field_size[static_cast<int>(entryType)];
            size_t fieldSize = fixedSize >= 0 ? fixedSize : LRead32(fileHandle);
            ...
            if (entryType == FieldTypes::Bitmap8bit)
            {
                ...
                bmpHeader = LReadDat8BitBmpHeader(fileHandle);
            ...
            else if (entryType == FieldTypes::Bitmap16bit)
            {
                ...
                if (fullTiltMode)
                {
                    zMapResolution = LRead8(fileHandle);
                    fieldSize--;
                ...
                zMapHeader = LReadDat16BitBmpHeader(fileHandle);
