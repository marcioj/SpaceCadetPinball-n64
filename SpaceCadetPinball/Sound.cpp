...
int Sound::Volume = 1.0f; // MIX_MAX_VOLUME;
...
void Sound::PlaySound(wav64_t *wavePtr, int time, TPinballComponent *soundSource, const char *info)
{
    if (MixOpen && wavePtr && enabled_flag)
    {
        static int current_channel = 0;
        wav64_play(wavePtr, current_channel);
...
wav64_t *Sound::LoadWaveFile(const std::string &lpName)
{
    if (!MixOpen)
        return nullptr;
    ...
    // Check if exists
    FILE *file;
    if ((file = fopen(lpName.c_str(), "r")) != NULL)
    {
        fclose(file);
    }
    else
    ...
    wav64_t *wav = new wav64_t;
    wav64_open(wav, lpName.c_str());
    return wav;
...
void Sound::FreeSound(wav64_t *wave)
{
    // if (MixOpen && wave)
    //     Mix_FreeChunk(wave);
}
