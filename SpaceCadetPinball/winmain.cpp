...
int winmain::WinMain(LPCSTR lpCmdLine)
{
    ...
    debugf("Game version: %s\n", Version);
    // debugf("Command line: %s\n", lpCmdLine);
    // debugf("Compiled with: SDL %d.%d.%d;", SDL_MAJOR_VERSION, SDL_MINOR_VERSION, SDL_PATCHLEVEL);
    // debugf(" SDL_mixer %d.%d.%d;", SDL_MIXER_MAJOR_VERSION, SDL_MIXER_MINOR_VERSION, SDL_MIXER_PATCHLEVEL);
    ...
    // if (!window)
    // {
    //     pb::ShowMessageBox(1, "Could not create window", SDL_GetError());
    //     return 1;
    // }
    ...
    // if (!renderer)
    // {
    //     pb::ShowMessageBox(1, "Could not create renderer", SDL_GetError());
    //     return 1;
    // }
    ...
    do
    {
        ...
        std::vector<const char *> searchPaths{{"rom:/"}};
        ...
        if (pb::init())
        {
            ...
            pb::ShowMessageBox(1, "Could not load game data", message.c_str());
            return 1;
        }
...
void winmain::MainLoop()
{
    ...
    while (true)
    {
        ...
        if (has_focus)
        {
            ...
            surface_t *disp = nullptr;
            if (UpdateToFrameCounter >= UpdateToFrameRatio && (disp = display_try_get()))
            {
...
void winmain::RenderUi()
{
    ...
    //                 {
    //                 case 0:
    //                     resolutionStringId = Msg::Menu1_UseMaxResolution_640x480;
    //                     break;
    //                 case 1:
    //                     resolutionStringId = Msg::Menu1_UseMaxResolution_800x600;
    //                     break;
    //                 case 2:
    //                     resolutionStringId = Msg::Menu1_UseMaxResolution_1024x768;
    //                     break;
    //                 }
...
int winmain::ProcessWindowMessages()
{
    ...
    if (pressed.raw)
    {
        ...
        {
            uint16_t input = 1 << i;
            if (pressed.raw & input)
            {
                pb::InputDown({InputTypes::GameController, input});
            }
        }
    }
    if (released.raw)
    {
        ...
        {
            uint16_t input = 1 << i;
            if (released.raw & input)
            {
                pb::InputUp({InputTypes::GameController, input});
            }
        }
    }
...
void winmain::memalloc_failure()
{
    ...
    pb::ShowMessageBox(1, caption, text);
    assertf(false, text);
    // std::exit(1);
}
...
void winmain::a_dialog()
{
    ...
    //                 auto tableRow = [](char *textA, char *textB)
    //                 {
    //                     ImGui::TableNextRow();
