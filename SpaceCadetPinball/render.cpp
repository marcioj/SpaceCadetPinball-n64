...
void render::build_occlude_list()
{
    ...
    for (auto mainSprite : sprite_list)
    {
        ...
        if (!mainSprite->DeleteFlag && mainSprite->BoundingRect.Width != -1)
        {
            ...
            {
                if (!refSprite->DeleteFlag && refSprite->BoundingRect.Width != -1 && maths::rectangle_clip(mainSprite->BoundingRect, refSprite->BoundingRect, nullptr))
                {
