    ...
    if (!demo_mode && !MainTable->CheatsUsed)
    {
        for (auto i = 0; i < playerCount; ++i)
        {
            ...
            if (position >= 0)
            {
                ...
                default:
                case 0:
                    playerName = get_rc_string(Msg::STRING127);
                    break;
                case 1:
                    playerName = get_rc_string(Msg::STRING128);
                    break;
                case 2:
                    playerName = get_rc_string(Msg::STRING129);
                    break;
                case 3:
                    playerName = get_rc_string(Msg::STRING130);
                    break;
                }
...
void pb::ShowMessageBox(uint32_t flags, LPCSTR title, LPCSTR message)
{
    // fprintf(flags == 1 ? stderr : stdout, "BL error: %s\n%s\n", title, message);
