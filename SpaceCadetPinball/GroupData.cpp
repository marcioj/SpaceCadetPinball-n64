...
void GroupData::AddEntry(EntryData *entry)
{
    ...
    {
        zmap_header_type *zMap = reinterpret_cast<zmap_header_type *>(entry->Buffer);
        int length = zMap->Height * zMap->Stride;

        for (int i = 0; i < length; i++)
        {
            zMap->ZPtr1[i] = __builtin_bswap16(zMap->ZPtr1[i]);
        }
    ...
    {
        int16_t *value = reinterpret_cast<int16_t *>(entry->Buffer);
        *value = __builtin_bswap16(*value);
        break;
    }
    case FieldTypes::ShortArray:
    {
        int shortArrLength = entry->FieldSize / 2;
        short *shortArr = reinterpret_cast<int16_t *>(entry->Buffer);
        for (int i = 0; i < shortArrLength; i++)
        {
            shortArr[i] = __builtin_bswap16(shortArr[i]);
        }
    ...
    {
        int floatArrLength = entry->FieldSize / 4;
        float *floatArr = reinterpret_cast<float *>(entry->Buffer);
        for (int i = 0; i < floatArrLength; i++)
        {
            floatArr[i] = bswapfloat(floatArr[i]);
        }
...
void DatFile::Finalize()
{
    if (!pb::FullTiltMode)
    {
        ...
        auto rcData = reinterpret_cast<MsgFont *>(asset_load("rom:/PB_MSGFT.bin", NULL));
        rcData->GapWidth = __builtin_bswap16(rcData->GapWidth);
        rcData->Unknown1 = __builtin_bswap16(rcData->Unknown1);
        rcData->Height = __builtin_bswap16(rcData->Height);
